...
- `[可用]` [superpowers-dsh](https://github.com/LayneChai/superpowers-dsh) 91 — Superpowers skills for DeepSeek Harness: TDD, debugging, planning, and collaboration skills adapted from obra/
...
- `[可用]` [dsh-claude-move](https://github.com/PerryLink/dsh-claude-move) 24 — DeepSeek Harness (dsh) plugin: migrate Claude Code sessions, memory, skills and CLAUDE.md into DSH with seamle
...
- `[可用]` [dsh-wanghong-handwritten-ppt](https://github.com/tjxj/dsh-wanghong-handwritten-ppt) 6 — 王虹学术手写风 PPT Skill for DeepSeek Harness · Notability-style HTML slides and PNG ex

## 🧠 记忆增强（20）

- `[可用]` [dsh-mnemon](https://github.com/omdsh-dev/dsh-mnemon) 385 — Cross-agent, local-first persistent memory plugin for DeepSeek Harness (DSH), powered by Mnemon
...
- `[可用]` [dsh-recall](https://github.com/fengshenx/dsh-recall) 0 — DSH 插件：recall 工具——模型可搜索并读取自己会话的完整事件日志，包括被压缩（compaction）遮蔽的内容；dsh plugin add 一条命令安装
...
- `[待定]` [dsh-undo-plugin](https://github.com/lire1131/dsh-undo-plugin) 160 — DSH plugin: snapshot & rollback your plugin/skin/settings configs
...
- `[可用]` [dsh-market-dsh-market](https://github.com/dsh-market/dsh-market) 4143 — —
...
- `[可用]` [DSH-better-sidebar](https://github.com/omdsh-dev/DSH-better-sidebar) 3661 — 一个侧边栏的完整工作台，支持三方拓展注册新侧边栏页面
- `[可用]` [dsh-TUI](https://github.com/ccch1mneyyy/dsh-TUI) 3092 — 解决DSH 官方尚无终端 TUI 痛点的补位之作，献给偏爱cli的各位极客：Claude Code 风格全屏交互终端插件——像素鲸鱼顶栏、实时工作状态行、思考流式展开、双击 Esc 回滚、上下文进度条 + TPS 仪表
- `[可用]` [dsh-ads](https://github.com/Nagi-ovo/dsh-ads) 628 — 把 DSH 变成 2005 年门户网站｜Parody ads, fake games, and popups for the DSH Web UI
...
- `[待定]` [dsh-tianshu-tui](https://github.com/huiliyi37/dsh-tianshu-tui) 280 — dsh-tianshu-tui — DeepSeek Harness terminal UI +harness workflow
...
- `[可用]` [dsh-genui](https://github.com/omdsh-dev/dsh-genui) 464 — GenUI for DeepSeek Harness: interactive UI components rendered inline in assistant replies via the dsh-ui fenc
- `[可用]` [dsh-vision-router](https://github.com/ysr666/dsh-vision-router) 1107 — Eyes for text-only DeepSeek Harness agents: built-in free vision chain (no key) + pixel-level vision tools (Q&
...
- `[可用]` [dsh-annotation](https://github.com/omdsh-dev/dsh-annotation) 123 — DSH Web 选中批注插件：选文字→批注→回车随消息发送；气泡隐藏批注块（零闪烁）；回复按 Annotation N 逐条对照（可悬浮芯片）
- `[不兼容]` [dsh-skill-viewer](https://github.com/Fishquito7/dsh-skill-viewer) 133 — DSH Web UI plugin: Skills settings section with hot enable/disable, delete and add（Web界面的skill管理工具）
- `[不兼容]` [heigeai-deepseek-harness-skin](https://github.com/HeiGeAi/deepseek-harness-skin) 52 — —
- `[不兼容]` [ui-status-label](https://github.com/alingalingling/ui-status-label) 47 — 把你鲸鱼娘思考时的 deep diving 自定义成任意你想要的样子
...
- `[待定]` [dsh-auto-continue](https://github.com/HsiangNianian/dsh-auto-continue) 103 — DSH Web UI plugin: auto-sends 「继续」 to resume requests interrupted by network errors or other non-human causes
...
- `[未测]` [dsh-plugin-subscriptions](https://github.com/V1ki/dsh-plugin-subscriptions) 371 — Use ChatGPT (Codex), Claude, and Grok (X Premium) subscriptions as DeepSeek Harness LLM providers — OAuth logi
...
- `[未测]` [TokenTracker](https://github.com/xiufengsun/TokenTracker) 1661 — Local-first AI token usage & cost tracker for 31 coding tools incl
...
- `[待定]` [dsh-agent-teams](https://github.com/NanmiCoder/dsh-agent-teams) 1724 — AgentTeams plugin for DeepSeek Harness
...
- `[待定]` [dsh_workflow](https://github.com/icetomoyo/dsh_workflow) 128 — 把Claude Code的UltraCode模式带给DSH，把 DSH 的一次性多 Agent 调度，升级为可生成、可保存、可治理、可观察、可恢复的 Workflow 层
...
- `[可用]` [dsh-chat-import](https://github.com/Nwflower/dsh-chat-import) 177 — 从Claude Code、Codex、Reasonix等Agent工具导入迁移历史消息，并在DeepSeek Harness(DSH)中继续对话
...
- `[可用]` [dsh-commandcode-provider](https://github.com/Mars-Sea/dsh-commandcode-provider) 283 — Unofficial DeepSeek Harness LLM provider plugin for Command Code: live model catalog, reasoning-effort support
...
- `[不兼容]` [franksong2702-dsh-codex-connect](https://github.com/franksong2702/dsh-codex-connect) 113 — —
...
- `[可用]` [billion-context-dsh](https://github.com/Tyan66666/billion-context-dsh) 98 — Model-driven context management (Active Context Pruning / ACP) for the DeepSeek Harness — the model decides wh
...
- `[可用]` [dsh-llm-codebuddy](https://github.com/Axiaohungry/dsh-llm-codebuddy) 41 — 在deepseek harness中使用workbuddy api，因为公司只提供workbuddy积分
...
- `[可用]` [dsh-auto-review](https://github.com/PerryLink/dsh-auto-review) 182 — Second-model AI auto-review for DeepSeek Harness approval requests: a read-only
...
- `[可用]` [dsh-github](https://github.com/PerryLink/dsh-github) 14 — GitHub integration for DeepSeek Harness: create PRs, review PRs in background jobs, read issues - every write
...
- `[可用]` [dsh-file-mentions](https://github.com/a903067276-rgb/dsh-file-mentions) 13 — Clickable file paths in DSH replies: Codex-style inline open, 📂 reveal in file m
...
- `[可用]` [foryourhealth111-pixel-vibe-skills](https://github.com/foryourhealth111-pixel/Vibe-Skills) 3339 — —
- `[可用]` [modlens](https://github.com/liustack/modlens) 3986 — The first vision plugin for DeepSeek Harness, and the vision bridge for every text-only coding agent
- `[不兼容]` [anionex-agent-vision-toolkit](https://github.com/Anionex/agent-vision-toolkit) 1205 — —
...
- `[不兼容]` [Abu-Cowork](https://github.com/PM-Shawn/Abu-Cowork) 380 — Open-source alternative to Claude Cowork — a local-first AI agent desktop app · multi-model · self-evolving sk
- `[可用]` [linhay-harmony-next-skills](https://github.com/linhay/harmony-next.skills) 350 — —
- `[不兼容]` [openma-ai-open-managed-agents](https://github.com/openma-ai/open-managed-agents) 285 — —
- `[可用]` [dsh-memory-evolve](https://github.com/csyangwen/dsh-memory-evolve) 318 — 为 DeepSeek Harness 带来「跨会话长期记忆 + 后台自我进化」能力的纯插件实现：五轨记忆 · git 分支感知 · 回合内自我审查 · 技能自我进化与技能管理器 · 四轨待办 · COI 调度 · 会话广 〔PR〕
...
- `[可用]` [bowenliang123-dsh-context](https://github.com/bowenliang123/dsh-context) 1434 — —
- `[不兼容]` [dsh-automation](https://github.com/titanwings/dsh-automation) 95 — DSH 自动化插件：让 Coding 任务按计划在全新 Agent Session 中运行，并由用户或 Agent 创建和管理定时任务
...
- `[可用]` [dsh-reverse-skill](https://github.com/dhicoc/dsh-reverse-skill) 148 — Complete reverse-skill (85 SKILL.md) as a DeepSeek Harness (dsh) Cordis plugin — reverse engineering, authoriz
...
- `[可用]` [dsh-auto-memory](https://github.com/Aik358/dsh-auto-memory) 71 — DSH 自动记忆插件:三层记忆(用户级/项目笔记/每日日志)自动注入与检索、每日反思、可视化面板与设置页,支持继承其他 AI 工具的历史记忆
...
- `[可用]` [dsh-godot-skill](https://github.com/akira399/dsh-godot-skill) 25 — Godot Engine 4.x 全栈游戏开发技能插件 for DeepSeek Harness (DSH) — registers the godot-4-development skill at runtime
...
- `[可用]` [green-dalii-dsh-plugin-dev-skill](https://github.com/green-dalii/dsh-plugin-dev-skill) 5 — —
...
- `[待定]` [dsh-background-agents](https://github.com/PerryLink/dsh-background-agents) 16 — Interactive long-session background agents for DeepSeek Harness: start a durable continuable child agent, watc
...
- `[可用]` [dsh-session-pin](https://github.com/PerryLink/dsh-session-pin) 9 — Pin sessions in the DeepSeek Harness (DSH) web sidebar - dual-face plugin with a hover pin badge, durable pinn
...
- `[未测]` [dsh-expression](https://github.com/yyh-001/dsh-expression) 93 — 找得到、发得出 —— DSH 表情包插件：语义搜图，只发真实文件，走 companion QQ 通道
...
- `[可用]` [dsh-codex-subscription](https://github.com/WSL043/dsh-codex-subscription) 76 — 在 DeepSeek Harness 中使用 ChatGPT/Codex 订阅，支持 OAuth 登录、额度与重置时间 \| ChatGPT/Codex subs
...
- `[可用]` [dsh-openpencil](https://github.com/ZSeven-W/dsh-openpencil) 174 — OpenPencil design preview and editing plugin for DSH
...
- `[可用]` [dsh-web-search-pro](https://github.com/anweat/dsh-web-search-pro) 67 — Enhanced, persistent web search plugin for DeepSeek Harness (multi-engine search, SQLite+LRU cache, platform b
...
- `[不兼容]` [dsh-openmaic](https://github.com/THU-MAIC/dsh-openmaic) 76 — OpenMAIC for DeepSeek Harness: classrooms, slides, interactive widgets, and Socratic teaching
...
- `[不兼容]` [dsh-mac-vision](https://github.com/Kevoyuan/dsh-mac-vision) 0 — On-device macOS OCR and Apple Vision for DeepSeek Harness — one native plugin wi
...
- `[不兼容]` [dsh-plugin-vision](https://github.com/tdf1995/dsh-plugin-vision) 4 — Vision for text-only LLMs in DeepSeek Harness (DSH): describe images / OCR / VQA
...
- `[可用]` [crafter-station-petdex](https://github.com/crafter-station/petdex) 4129 — —
- `[待定]` [small-tailqwq-dsh-deep-whale](https://github.com/Small-tailqwq/dsh-deep-whale) 2129 — —
- `[可用]` [whale-girl](https://github.com/vlln/whale-girl) 325 — DSH Web GUI 桌面宠物插件（QQ 宠物形态）：右下角悬浮、可拖拽/投喂/玩耍的积累型伙伴
- `[不兼容]` [pc2005-cloud-dsh-pet](https://github.com/PC2005-cloud/dsh-pet) 677 — —
...
- `[可用]` [dsh-stickers](https://github.com/william-jin-cmu/dsh-stickers) 23 — DSH WebUI sticker plugin for bidirectional user and agent reactions
...
- `[可用]` [dsh-whale-pet](https://github.com/lglglglgy/dsh-whale-pet) 2 — dsh-whale-pet
...
- `[可用]` [dsh-memento](https://github.com/PerryLink/dsh-memento) 103 — Bounded, layered, approval-gated, auditable cross-session memory for DeepSeek Harness (capability seam: ctx.me
...
- `[不兼容]` [anywhere-labs-deepseek-harness-desktop](https://github.com/anywhere-labs/deepseek-harness-desktop) 27578 — —
- `[不兼容]` [dataelement-dsh-desktop](https://github.com/dataelement/dsh-desktop) 7463 — —
- `[可用]` [hairyf-deepseek-harness-desktop](https://github.com/hairyf/deepseek-harness-desktop) 2269 — —
- `[可用]` [sepinetam-mcp-for-stata](https://github.com/SepineTam/mcp-for-stata) 260 — —
- `[不兼容]` [lum1104-dsh-browser](https://github.com/Lum1104/dsh-browser) 688 — —
- `[未测]` [Bigfish](https://github.com/turtle2209/Bigfish) 314 — Bigfish —— DeepSeek Harness 的第三方桌面端，内置 Node 运行时，双击即用，附带桌面萌宠
- `[待定]` [Deepseek-Harness-Desktop](https://github.com/ChisaAlter/Deepseek-Harness-Desktop) 170 — DSH桌面端，支持主题和背景图等多种个性化配置
- `[不兼容]` [ningbainb-deepseek-harness-desktop](https://github.com/ningbainb/deepseek-harness-desktop) 619 — —
...
- `[不兼容]` [kelai141-dsh-mobile-apk](https://github.com/kelai141/dsh-mobile-apk) 469 — —
...
- `[不兼容]` [ego-browser](https://github.com/Fisfzy/ego-browser) 180 — DSH（DeepSeek Harness）插件：把 ego-lite 浏览器（给 AI Agent 用的 Chromium）接入 HARNESS——13 个结构化 ego_* 工具（文本语义快照、语义定位点击、表单填充、
- `[可用]` [cc1252-deepseek-harness-desktop](https://github.com/cc1252/deepseek-harness-desktop) 19 — —
- `[可用]` [flymysql-dsh-remote](https://github.com/flymysql/dsh-remote) 85 — —
- `[不兼容]` [dsh-mobile](https://github.com/lehhair/dsh-mobile) 25 — —
- `[可用]` [liguobao-dsh-desktop](https://github.com/liguobao/dsh-desktop) 38 — —
- `[可用]` [runzhliu-deepseek-harness-docker](https://github.com/runzhliu/deepseek-harness-docker) 87 — —
...
- `[待定]` [sorsama-deepseek-harness-mobile](https://github.com/sorsama/deepseek-harness-mobile) 83 — —
...
- `[可用]` [ankocd-dsh-server-deployment](https://github.com/AnkoCD/dsh-server-deployment) 33 — —
...
- `[不兼容]` [michengai-deepseek-harness-desktop](https://github.com/MichengAI/deepseek-harness-desktop) 87 — —
- `[不兼容]` [minilaba-deepseek-harness-desktop](https://github.com/MiniLaba/deepseek-harness-desktop) 1 — —
- `[可用]` [mrbbbaixue-dsh-desktop](https://github.com/mrbbbaixue/dsh-desktop) 5 — —
...
- `[可用]` [dsh-download-monitor](https://github.com/keepermttl/dsh-download-monitor) 0 — DSH Web GUI download monitor plugin
...
- `[可用]` [alaliqing-claude-paper](https://github.com/alaliqing/claude-paper) 338 — —
...
- `[可用]` [deepseek-ai-deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) 229080 — —
- `[可用]` [devin-axis-ipollowork](https://github.com/Devin-AXIS/iPolloWork) 6339 — —
- `[不兼容]` [awesome-dsh-plugin-awesome-dsh-plugin](https://github.com/awesome-dsh-plugin/awesome-dsh-plugin) 16181 — —
- `[不兼容]` [imsai-sh-zhuzhiliao](https://github.com/imsai-sh/zhuzhiliao) 2882 — —
...
- `[不兼容]` [0xsline-awesome-deepseek-harness](https://github.com/0xsline/awesome-deepseek-harness) 1076 — —
- `[可用]` [mnemon-dev-mnemon](https://github.com/mnemon-dev/mnemon) 587 — —
- `[不兼容]` [electricitysheep-dsh-handbook](https://github.com/Electricitysheep/dsh-handbook) 792 — —
- `[可用]` [morluto-rea](https://github.com/morluto/rea) 412 — —
- `[可用]` [bruc3van-awesome-dsh-plugin](https://github.com/bruc3van/awesome-dsh-plugin) 344 — —
...
- `[可用]` [modsearch](https://github.com/liustack/modsearch) 506 — The web plugin for DeepSeek Harness, and the search bridge for every model without native web access
- `[可用]` [sikao-engine-kimix](https://github.com/Sikao-Engine/KimiX) 111 — —
- `[不兼容]` [hikariming-dshfind](https://github.com/hikariming/dshfind) 262 — —
- `[不兼容]` [pulseaiclub-phi](https://github.com/pulseaiclub/phi) 433 — —
- `[可用]` [libukai-awesome-deepseek-harness](https://github.com/libukai/awesome-deepseek-harness) 268 — —
- `[不兼容]` [dominic789654-awesome-deepseek-harness](https://github.com/Dominic789654/awesome-deepseek-harness) 342 — —
...
- `[可用]` [ayase34-gal-view](https://github.com/Ayase34/gal-view) 180 — —
- `[可用]` [pingfanfan-hello-dsh](https://github.com/pingfanfan/hello-dsh) 95 — —
- `[可用]` [lyn-77-promentor](https://github.com/Lyn-77/ProMentor) 75 — —
...
- `[不兼容]` [beancookie-awesome-dsh-plugin](https://github.com/beancookie/awesome-dsh-plugin) 147 — —
- `[可用]` [dsh-cost-meter](https://github.com/Han-1413141/dsh-cost-meter) 312 — DeepSeek Harness 会话费用统计插件:本会话费用、当日费用、历史记录与官方价格同步
...
- `[可用]` [blender](https://github.com/CheshireJCat/blender) 31 — DeepSeek Harness plugin for complete Blender 3D modeling, reconstruction, rendering, validation, and export wo
- `[可用]` [dqsjqian-aria](https://github.com/dqsjqian/Aria) 90 — —
...
- `[可用]` [modusensus-dsh-mneme](https://github.com/modusensus/dsh-mneme) 112 — —
...
- `[可用]` [dsh-status-rotator](https://github.com/01Virex/dsh-status-rotator) 86 — A DeepSeek Harness (dsh) web plugin that replaces the "Deep diving…" turn-status label with phase-aware, typew
- `[不兼容]` [fakechris-dsh-harness-ops](https://github.com/fakechris/dsh-harness-ops) 14 — —
- `[可用]` [imsai-sh-awesome-deepseek-harness-plugins](https://github.com/imsai-sh/awesome-deepseek-harness-plugins) 242 — —
...
- `[可用]` [dsh-agy](https://github.com/chaos-03x/dsh-agy) 41 — Google Antigravity (agy) OAuth auth + model access plugin for DeepSeek Harness: multi-account pool, 429 rotati
...
- `[可用]` [pi2dsh](https://github.com/weijiafu14/pi2dsh) 195 — Bridge the Pi and DeepSeek Harness ecosystems: one Pi Host ABI runs unmodified Pi extensions as native DSH plu
...
- `[可用]` [deepseek-harness-acp](https://github.com/openma-ai/deepseek-harness-acp) 32 — ACP server implementation for DeepSeek harness
...
- `[可用]` [dsh-passwords](https://github.com/slywalker2006/dsh-passwords) 61 — dsh-passwords: DeepSeek Harness login gateway - first-run setup, at-rest encryption, brute-force lockout, audi
...
- `[待定]` [TokenLedger](https://github.com/zh667/TokenLedger) 200 — Token usage accounting for DeepSeek Harness, reconciled against New API and Sub2API relay-site billing
...
- `[可用]` [perrylink-dsh-doublecheck](https://github.com/PerryLink/dsh-doublecheck) 34 — —
...
- `[不兼容]` [A-persona-plugin-for-DeepSeek-Harness](https://github.com/chenjunda0018-sketch/A-persona-plugin-for-DeepSeek-Harness) 0 — switch DSH between a Classical Chinese scholar, a Northeastern buddy, and a talk
- `[不兼容]` [dsh-toolbelt](https://github.com/cking000bigdemon/dsh-toolbelt) 0 — Eight DeepSeek Harness plugins: persona, language guard, per-request vision fall
